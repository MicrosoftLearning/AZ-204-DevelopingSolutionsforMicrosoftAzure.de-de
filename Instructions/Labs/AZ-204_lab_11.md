...
1. Führen Sie auf dem Blatt **Web-App erstellen** auf der Registerkarte **Grundlagen** die folgenden Aktionen aus, und wählen Sie dann die Registerkarte **Überwachen** aus:
    ...
    | Dropdownliste **Laufzeitstapel** | Wählen Sie **.NET 6 (LTS)** aus. |
    | Abschnitt **Betriebssystem** |  Wählen Sie **Windows** aus. |
    | Dropdownliste **Region**  |  Wählen Sie dieselbe Region aus, die Sie als Standort der Ressource **Anwendungsinstanz** ausgewählt haben. |
...
1. Führen Sie im Abschnitt **Konfiguration** die folgenden Aktionen aus:
    ...
    b.  Notieren Sie den Wert, der den Schlüssel **APPLICATIONINSIGHTS_CONNECTION_STRING** darstellt. Dieser Wert wurde beim Erstellen der Web-API-Ressource automatisch festgelegt.
...
1. Scrollen Sie nach unten, bis die **Scaleout-Methode** angezeigt wird. Wählen Sie die Option **Regelbasiert** aus, und klicken Sie anschließend auf **Regelbasierte Skalierung verwalten**.
...
1. Führen Sie den folgenden Befehl aus, um Version 2.20.0 von **Microsoft.ApplicationInsights** aus NuGet in das aktuelle Projekt zu importieren:
...
1. Führen Sie den folgenden Befehl aus, um Version 2.20.0 von **Microsoft.ApplicationInsights.AspNetCore** aus NuGet zu importieren:
...
1. Führen Sie an der Terminaleingabeaufforderung den folgenden Befehl aus, um Version 2.20.0 von **Microsoft.ApplicationInsights.PerfCounterCollector** aus NuGet in das aktuelle Projekt zu importieren:
...
1. Suchen Sie im **Editor**-Bereich den folgenden Code in Zeile 19, und löschen Sie ihn:
...
1. Fügen Sie in der Datei **Program.cs** ab Zeile 9 die folgenden beiden Zeilen ein, um das Profiler-Tool aktivieren, indem es zusammen mit Application Insights in der Dienstsammlung registriert wird:
...
1. Navigieren Sie in dem geöffneten Browserfenster zur URL `https://localhost`, und fügen Sie den relativen Pfad **/weatherforecast** Ihrer Web-API hinzu.
    
    > **Hinweis**: Die vollständige URL lautet `https://localhost:[port-number]/weatherforecast`. Dabei gibt der Platzhalter `[port-number]` die Nummer des Ports an, an dem die Web-App über das HTTPS-Protokoll erreichbar ist.
...
1. Schließen Sie das Browserfenster, in dem die von `https://localhost:[port-number]/weatherforecast` generierte Seite angezeigt wird.
...
1. Navigieren Sie im **Visual Studio Code**-Fenster im **Explorer**-Bereich zum Verzeichnis **bin\Debug\net6.0**.
    ...
    > **Hinweis**: Um zu überprüfen, ob Sie sich im Verzeichnis **bin\Debug\net6.0** befinden, wählen Sie in Visual Studio Code im Menü „Datei“ die Option „Ordner öffnen“ aus, und navigieren Sie zu **Allfiles (F):\\Allfiles\\Labs\\11\\Starter\\Api\\bin\\Debug\\net6.0**, und wählen Sie dann **Ordner auswählen** aus.
...
1. Führen Sie an der Terminaleingabeaufforderung den folgenden Befehl aus, um sicherzustellen, dass das aktuelle Verzeichnis, in dem sich die Bereitstellungsdateien befinden, auf **Allfiles (F):\\Allfiles\\Labs\\11\\Starter\\Api\\bin\\Debug\\net6.0** festgelegt ist:
...
1. Führen Sie den folgenden Befehl aus, um die zuvor in dieser Aufgabe erstellte **api.zip**-Datei für die Web-API bereitzustellen, deren Name Sie im vorherigen Schritt ermittelt haben. Wählen Sie **Y** aus, wenn Sie vom Befehl aufgefordert werden:
...
### Übung 4: Application Insights-Protokollierung mit .NET

#### Aufgabe 1: Konfigurieren der Protokollierung für eine .NET-API-App
...
1. Führen Sie den folgenden Befehl aus, um Version 2.20.0 von **Microsoft.Extensions.Logging.ApplicationInsights** aus NuGet in das aktuelle Projekt zu importieren:
...
1. Navigieren Sie im **Visual Studio Code**-Fenster im **Explorer**-Bereich zum Verzeichnis **Controller**, und öffnen Sie dann die Datei **WeatherForecastController.cs**.

1. Überprüfen Sie den Inhalt der Datei, und beachten Sie, dass sie eine Konstruktorinjektion für die generische `ILogger<WeatherForecastController>`-Schnittstelle enthält.
...
1. Ersetzen Sie die Definition der `public IEnumerable<WeatherForecast> Get()`-Methode in der Datei **WeatherForecastController.cs** durch den folgenden Inhalt:
...
#### Aufgabe 2: Testen der Protokollierung einer .NET Core-API-App
...
1. Navigieren Sie im geöffneten Browserfenster zu der von Ihnen identifizierten URL, fügen Sie den relativen Pfad **/weatherforecast** Ihrer Web-API hinzu, und aktualisieren Sie dann die Seite mehrmals.
...
#### Aufgabe 3: Überprüfen der Application Insights-Protokollierung
