...
1. Wählen Sie auf der Taskleiste erneut das Symbol **Windows-Terminal** aus, um eine neue Instanz der Anwendung **Windows-Terminal** zu öffnen.
1. Führen Sie den folgenden Befehl an der Eingabeaufforderung aus, um den REST-API-Aufruf **POST** mit `http://localhost:7071/api/echo` zu testen, wobei der HTTP-Anforderungstext auf einen numerischen Wert von **3** festgelegt ist:
