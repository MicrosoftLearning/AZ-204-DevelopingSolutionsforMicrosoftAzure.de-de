...
Schließlich legen Sie die Konsistenzebene für Ihre Cosmos DB-Instanz fest und implementieren eine Azure-Funktion für Änderungsfeedbenachrichtigungen.
...
1. Klicken Sie anschließend auf **Freigegebene Zugriffstoken**, und generieren Sie ein Zugriffstoken mit den Standardeinstellungen, indem Sie auf **SAS-Token und URL generieren** klicken. Notieren Sie sich den Wert des **Bob-SAS-Tokens**, da Sie diesen später benötigen.
...
1. Aktualisieren Sie die Eigenschaften **BlobContainerUrl** und **BlobSASToken**, indem Sie ihren Wert auf die Eigenschaft **URL** des Azure Storage-Blobcontainers mit dem Namen **images** festlegen, die Sie sich zuvor in diesem Lab notiert haben.

1. Speichern Sie die Datei **appsettings.json**, und schließen Sie sie.

   > **Hinweis**: Wählen Sie **Überschreiben** aus, wenn Ihnen eine Eingabeaufforderung angezeigt wird, die besagt, dass die Datei schreibgeschützt ist.

1. Klicken Sie im Fenster **Visual Studio Code** im Explorer-Bereich mit der rechten Maustaste auf den Ordner **AdventureWorks.Context** und wählen Sie anschließend **In integriertem Terminal öffnen** aus.
...
1. Führen Sie an der Terminaleingabeaufforderung den folgenden Befehl aus, um das **AdventureWorks.Context**-Projekt zu erstellen:
