...
1.  Wählen Sie auf dem Blatt **API-Option auswählen** die Option **Core (SQL) - empfohlen** aus.
...
1.  Navigieren Sie im Azure-Portal zurück zum Blatt **Azure Cosmos DB**, und wählen Sie in der Liste der Cosmos DB-Konten das neu erstellte Azure Cosmos DB-Konto **polycosmos** _[Ihr Name]_ aus.
...
1.  Wählen Sie **im Visual Studio Code** einen leeren Bereich des **Explorer**-Bereichs aus, aktivieren Sie das Kontextmenü, und wählen Sie dann **Im integrierten Terminal öffnen** aus.

1.  Überprüfen Sie an der Terminaleingabeaufforderung, ob das aktuelle Verzeichnis auf **AdventureWorks** festgelegt ist (ändern Sie es in dieses Verzeichnis, falls dies nicht der Fall ist), und führen Sie dann den folgenden Befehl aus, um den Terminalkontext in den Ordner **AdventureWorks.Upload** umzuschalten:
...
1.  Legen Sie in Zeile 18 den Wert von **PrimaryKey** fest, indem Sie die leere Zeichenfolge durch **"/Category"** ersetzen.
...
1.  Führen Sie an der Terminaleingabeaufforderung den folgenden Befehl aus, um alle fehlenden NuGet-Pakete wiederherzustellen und das Projekt im Ordner zu erstellen:
...
1.  Navigieren Sie im Azure-Portal zurück zum Blatt **Azure Cosmos DB**, und wählen Sie in der Liste der Cosmos DB-Konten das Azure Cosmos DB-Konto **polycosmos** _[Ihr Name]_ aus, das Sie zuvor in diesem Lab erstellt haben.
